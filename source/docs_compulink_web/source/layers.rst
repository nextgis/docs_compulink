...
Одним из основных компонентов ПО NextGIS Web Compulink является слой. Слой - это растровое 
...
.. note:: 
   ...
   загрузки растра в NextGIS Web Compulink и обрезки по альфа-каналу. 
...
.. note:: 
   В NextGIS Manager эту операцию можно сделать проще. В программе есть функционал
   загрузки в NextGIS Web Compulink векторных файлов разных форматов, без необходимости 
...
#. Ввести название таблицы (слоя PostGIS). 
	...
	Отображение таблиц и представлений не входит в задачи NextGIS Web Compulink. Для просмотра можно воспользоваться :program:`NextGIS Manager` или :program:`PgAdmin`.
#. Ввести поле ID. 
...
Программное обеспечение NextGIS Web Compulink поддерживает добавление таблиц, в которых в 
...
точек. В этом случае в NextGIS Web Compulink нужно добавить три отдельных слоя для каждого 
...
В :program:`NextGIS Web Compulink` нельзя указывать условия отбора записей из слоя (SQL конструкция WHERE). 
...
Программное обеспечение NextGIS Web Compulink является клиентом WMS. Для подключения слоя WMS 
...
Программное обеспечение NextGIS Web Compulink может работать как сервер WMS. По этому протоколу 
...
Cлой NextGIS Web Compulink можно добавлять в настольные, мобильные и веб ГИС несколькими способами.


Подключение к WMS
^^^^^^^^^^^^^^^^^^^^^^^^^

NextGIS Web Compulink является сервером WMS. Соответственно подключить его слои как WMS можно 
...
Конкретные слои NextGIS Web Compulink можно подключать как WMS. Для использования их через утилиты GDAL нужно создать для 
...
Конкретные слои NextGIS Web Compulink можно подключать как TMS. Для этого нужно создать для 
...
NextGIS Web Compulink может работать как сервер WFS. По этому протоколу сторонние программы 
...
С учетом опыта использования NextGIS Web Compulink рекомендуется следующая типовая структура 
