...
ГИС Компьюлинк предоставляет пользователю возможность добавлять:
...
Благодаря этим утилитам :program:`ГИС Компьюлинк` может читать и записывать разные 
...
различных интернет-сервисов в проект ГИС Компьюлинк. 
...
Дальнейшая работа со слоями PostGIS осуществляется в :program:`ГИС Компьюлинк` точно 
...
Можно добавлять слои по отдельности. В этом случае в :program:`ГИС Компьюлинк` слои 
будут видны как отдельные. Можно выделить несколько слоев, тогда они будут отдаваться 
с сервера как один слой. Дальнейшая работа со слоями WMS осуществляется в :program:`ГИС Компьюлинк` 
...
Из :program:`ГИС Компьюлинк` можно работать с NextGIS Web напрямую. Можно смотреть 
...
В :program:`ГИС Компьюлинк` есть возможность работы с проекциями. У каждого слоя данных 
есть своя система координат (в которой хранятся данные), как правило она записана 
в самом файле. Почти всегда в :program:`ГИС Компьюлинк` используется функция "преобразования 
...
В :program:`ГИС Компьюлинк` реализована возможность работы с проекциями. Проекция 
...
Все проекции в :program:`ГИС Компьюлинк` основаны на базе идентификаторов European Petroleum Group (ESPG) и Institut Geographique National of France (IGNF) и в значительной степени абстрагированы 
...
на уровне слоя или проекта. Для слоёв PostGIS :program:`ГИС Компьюлинк` использует 
...
:program:`ГИС Компьюлинк` создаёт новые проекты с использованием системы координат 
...
ГИС Компьюлинк поддерживает перепроецирование растровых и векторных слоёв "на лету", но по 
...
2. Система координат - список проекций, поддерживаемых ГИС Компьюлинк, включая географические,
...
с используемыми в ГИС Компьюлинк.
...
диалог отрисовки уникальными значениями из демонстрационного набора данных ГИС Компьюлинк:
...
добавлять этот слой как новый, то ГИС Компьюлинк подхватит и его стиль тоже.
...
способ для распространения географической информации созданной в :program:`ГИС Компьюлинк`, 
...
Перед началом работы в компоновщике карты нужно добавить в :program:`ГИС Компьюлинк` 
...
объекты в область компоновки: текущую карту из :program:`ГИС Компьюлинк`, надписи, 
...
По нажатию кнопки Текущий охват - охват выставится такой же, как у основного окна :program:`ГИС Компьюлинк`. 
По нажатию кнопки Установить охват для основной карты - охват основной карты выставится 
такой же, как у карты из макета. 
Охват сохраняется в макете, и изменения в основном окне :program:`ГИС Компьюлинк` 
