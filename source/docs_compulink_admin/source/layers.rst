...
Одной из основных сущностей NextGIS Web является слой. Слой - это растровое
изображение или векторный файл (таблица БД). Все данные хранятся в слоях.
Для объединения слоев в виде карты необходимо настроить стиль (или набор стилей) отображения слоя.
Стили могут настраиваться только для векторных слоев.


Специфическими ресурсами для системы Компьюлинк Мониторинг Строительства являются:
...
.. note::
   Стандартное использование системы Компьюлинк Мониторинг Строительства
   не предусматривает ручное создание и изменение ресурсов общих типов
   (слои, стили, сервисы и т.д.). Как правило администратор создает Проекты
   и объекты строительства.



.. _ngw_create_comp_project:

Проект
------
Является специальном типом ресурса, который может содержать один или несколько объектов строительства,
а также дополнительные необходимые ресурсы. Проекта не может содержать в себе другие проекты строительства
на любых вложенных уровнях.

Для создания нового Проекта перейдите в группу, где необходимо его создать. В
блоке операций выберите :menuselection:`Создать ресурс --> Проект`. Откроется окно,
представленное на :numref:`admin_layers_create_project_resourse_description`.


.. figure:: _static/admin_layers_create_project_resourse_description.png
   :name: admin_layers_create_project_resourse_description
   :align: center
   :width: 16cm

   Окно создания Проекта.

Необходимо ввести наименование проекта, которое будет отображаться в административном веб интерфейсе,
а также в дереве Проектов\ОС основной карты.
...
Является специальном типом ресурса, который содержит необходимые преднастроенные векторные слои и WFS сервис.
Преднастроенные ресурсы создаются в момент создания Объекта строительства.
Данный тип ресурса не может содержать в себе другие проекты и объекты строительства на любых вложенных уровнях.

Для создания нового Объекта строительства перейдите в проект или группу, где необходимо его создать. В
блоке операций выберите :menuselection:`Создать ресурс --> Объект строительства`. Откроется окно,
представленное на :numref:`admin_layers_create_os_resourse_description`.

.. figure:: _static/admin_layers_create_os_resourse_description.png
   :name: admin_layers_create_os_resourse_description
   :align: center
   :width: 16cm

   Окно создания Объекта строительства.


Необходимо ввести наименование объекта строительства, которое будет отображаться в административном веб интерфейсе,
а также в дереве Проектов\ОС основной карты.

.. _ngw_create_comp_dict:

Справочник
----------

Является специальном типом ресурса, который содержит простой справочник ключ-значение. Данные типы ресурсов используются в мобильном
Данный тип ресурса не может содержать в себе другие типы ресурсов.

.. note::
   Стандартное использование системы Компьюлинк Мониторинг Строительства
   не предусматривает ручное создание и изменение ресурсов типа справочник.
   Все необходимые справочники созданы заранее при первичном наполнении системы и расположены в дочернем ресурсе корневого элемента:
   Основная группа ресурсов -> Справочники. Для корректной работы системы группа со служебными сравочниками должна иметь ключ dictionary_group.
   Удаление или редактирование данных справочников может привести к некорректному поведению системы.

Для создания нового Объекта строительства перейдите в группу, где необходимо его создать. В
блоке операций выберите :menuselection:`Создать ресурс --> Справочник`. Откроется окно,
представленное на :numref:`admin_layers_create_lookup_resourse_description`.

.. figure:: _static/admin_layers_create_lookup_resourse_description.png
   :name: admin_layers_create_lookup_resourse_description
   :align: center
   :width: 16cm
...
Необходимо ввести наименование объекта строительства, которое будет отображаться в административном веб интерфейсе.
Так же необходимо заполнить ключи и значения на вкладке 'Справочник', как отображено на :numref:`admin_layers_create_lookup_resourse_table`.

.. figure:: _static/admin_layers_create_lookup_resourse_table.png
   :name: admin_layers_create_lookup_resourse_table
   :align: center
   :width: 16cm

   Окно ввода и редактирования значений справочника.
